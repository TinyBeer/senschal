...
`config/const.go` 中的导出常量与变量遵循以下约定：

1. **风格**：PascalCase，不使用下划线或 ALL_CAPS
2. **缩写展开**：`Def` → `Default`，`Conf`/`Cfg` → `Config`
3. **目录名常量**：`{Scope}DirName`，如 `ConfigDirName`、`SSHConfigDirName`
4. **计算路径变量**：`{Topic}Dir`，如 `ConfigDir`、`SSHKeyDir`、`DockerImageDir`
5. **特殊值**：`TplSettingName`（模板设置文件名），`DefaultDataDir`（数据根目录），`DefaultGenerateDir`（代码生成输出目录）

示例：旧的 `SSH_Conf_Dir_Name`/`SSH_CFG_DIR`/`SSH_KEY_DIR` → `SSHConfigDirName`/`SSHConfigDir`/`SSHKeyDir`
